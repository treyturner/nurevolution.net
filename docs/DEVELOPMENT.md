...
The canonical local and CI gate is **`pnpm verify`**, including local Docker integration tests (`pnpm test:delivery`). Use the workspace's configured Docker socket; test HTTP access through published container ports is a separate requirement. See [Docker socket and test HTTP setup](DEPLOYMENT.md#release-and-host-contracts) before setting an address override. The gate prepares its generated prerequisites and builds both applications without a hand-started server. It does not install dependencies, rewrite maintained files, accept snapshots, or contact the legacy site.
...
The [Verify workflow](../.github/workflows/verify.yml) runs on pull requests and pushes to `main`. A small `changes` job compares the complete PR diff from its merge base or the push's before/after commits. If every changed path ends in `.md`, the existing `verify` check runs only Markdown formatting with the exact Prettier version declared in `package.json`, using `pnpm dlx` without installing the application dependencies. This honors the repository's Prettier configuration and ignore rules. The expensive jobs and image/release publication are skipped at job or step level, so Markdown-only PRs still report a passing or failing formatting result. CodeQL retains its independent triggers.

Any non-Markdown change runs full verification, including workflow/configuration changes and JSON under `docs/` that supplies migration inputs or test evidence. Both paths of renames are considered; unavailable or empty diffs default to full verification. The `verify` check also fails if scope detection fails. Full verification uses Ubuntu 24.04, the declared Node/Python runtimes, a frozen installation, all three browsers, and exactly `pnpm verify`. Actions are pinned to verified commit SHAs, verification permissions are read-only, and available coverage/browser artifacts are retained for 14 days. PR verification needs no deployment secrets. After successful full main verification, a separate job publishes the exact tested application/Caddy images; manual production promotion is enabled and uses environment-scoped credentials. A verified application change publishes artifacts; it does not automatically deploy production. Separate Verify jobs also audit playback indexing, container image identity, temporary Headscale clients, and Headscale backup/restore tooling. A local successful run is separate evidence from an actual remote Actions run.
