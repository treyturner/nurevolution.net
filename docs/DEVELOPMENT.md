...
Shared links and RSS resource URLs keep the canonical production origin in workspace previews. To test a copied timestamp before deployment, retain its episode path and query but substitute the authenticated preview origin. A preview's canonical RSS chapter/artwork references do not make undeployed production resources available. Actual podcast-client acceptance is separate from the automated resource checks and remains pending until those resources are reachable by the client. Do not change the canonical feed identity to make a preview subscription work.
