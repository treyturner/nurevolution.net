...
The local resource-support slice implements `/chapters/<slug>.json` and `/episode-artwork/v1-<source-sha256>.jpg`. The retained support commit (`e1013e0`) leaves RSS unchanged. Subsequent commits add item-level `itunes:image` and eligible `podcast:chapters` references. PR #56 merged both stages, but the historical support commit never became the head of a main push and has no independently published release. The owner authorized the phased production rollout on 2026-09-18. The preparation release temporarily restores the pre-advertisement feed projection, serializer, and matching checks while preserving the current player, chapter endpoints, and artwork. Publish it through normal main verification, deploy and retain it, then promote the verified PR #56 merge release and restore advertisement on main. See the [rollout record](operations/m8-rollout.md). JSON chapters use version `1.2.0`, retain fractional starts, and omit unknown timestamps; untimed, unknown, draft, and future episodes return generic non-cacheable 404s. GET/HEAD share representation metadata, weak SHA-256 ETags support 304, and successful responses use `public, max-age=60, must-revalidate`, `nosniff`, and anonymous CORS. Read/serialization failures return a generic non-cacheable 503. Other methods return 405.
...
After advertisement, rollback targets must continue serving chapter and previously advertised artwork paths. Do not revert to a pre-support release that would strand cached feed references. Existing GUID/enclosure protection remains mandatory. The owner authorized both deployment phases on 2026-09-18; record their actual results before declaring rollout complete. Earlier all-episode Podcast Addict load/seek acceptance does not establish support for these new chapter/artwork fields.
