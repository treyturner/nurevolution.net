...
M9 must review the 60-second HTTP cache window, any intermediary caches, client polling, and publication-boundary behavior before claiming scheduled release precision. The deployed M8 implementation emits episode artwork and chapters from canonical metadata; HTTP verification passed, while physical podcast-client acceptance remains pending. See the [agreed M8 scope](milestones/M08-listening-and-feed-enhancements.md).
...
M8's phased production rollout completed on 2026-09-18. The resource-support release `4d0100c` made `/chapters/<slug>.json` and `/episode-artwork/v1-<source-sha256>.jpg` available while preserving the previous feed bytes. The subsequent promotion of the verified PR #56 merge release `4ca5c3d` enabled item artwork and chapter references. The support release is retained as production's compatible rollback target. The historical `e1013e0` support commit was never a main push; PR #57 created the independently verified support artifact without weakening publication provenance. See the [rollout record](operations/m8-rollout.md). JSON chapters use version `1.2.0`, retain fractional starts, and omit unknown timestamps; untimed, unknown, draft, and future episodes return generic non-cacheable 404s. GET/HEAD share representation metadata, weak SHA-256 ETags support 304, and successful responses use `public, max-age=60, must-revalidate`, `nosniff`, and anonymous CORS. Read/serialization failures return a generic non-cacheable 503. Other methods return 405.
...
The completed rollout followed this order:
