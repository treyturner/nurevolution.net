...
The local resource-support slice implements `/chapters/<slug>.json` and `/episode-artwork/v1-<source-sha256>.jpg`. That support commit (`a9a3231`) leaves RSS unchanged. The subsequent local `feat/m8-feed-advertisement` branch adds item-level `itunes:image` and eligible `podcast:chapters` references. Neither slice is deployed. JSON chapters use version `1.2.0`, retain fractional starts, and omit unknown timestamps; untimed, unknown, draft, and future episodes return generic non-cacheable 404s. GET/HEAD share representation metadata, weak SHA-256 ETags support 304, and successful responses use `public, max-age=60, must-revalidate`, `nosniff`, and anonymous CORS. Read/serialization failures return a generic non-cacheable 503. Other methods return 405.
