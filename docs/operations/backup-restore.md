...
The first command prompts invisibly and creates root-only `/etc/nurevolution/discord-webhook`; it refuses to overwrite an existing URL. The second sends one clearly labelled setup message. The URL is a credential and must stay out of Git, shell arguments, and logs. The notifier accepts the HTTPS Discord webhook endpoint, rejects redirects, suppresses mentions, and sends only a brief host/status message. It does not upload backup logs or secrets. Failed delivery retries up to three service starts within five minutes; webhook delivery failure remains visible as a systemd failure.
