...
The owner selected MinIO and confirmed backups after content changes plus weekly, retaining four weekly and three monthly snapshots. The owner supplied successful creation output for the `nurevolution-backups` bucket and `nurevolution-backup` policy/user; the user is enabled with only that policy and no group memberships. Droplet credentials, repository initialization, the complete 156-asset archive restore, and Discord failure notifications are verified. Application/certificate recovery and the normal backup service passed. Its weekly timer is enabled and survived the 2026-09-13 droplet reboot; the September 13 scheduled invocation completed successfully with a passing repository check, verified from the service journal on September 18. See the [acceptance record](acceptance-2026-09-18.md#scheduled-production-backup).
...
The current repository is initialized. A [small live fixture rehearsal](../milestones/evidence/M05-minio-fixture-restore.json) passed upload, full data verification, restoration with exact hashes, scope checks, and test-data cleanup from the droplet. That fixture was cleaned up. The later [full archive rehearsal](../milestones/evidence/M05-minio-archive-restore.json) was followed by successful full application/edge backups and independent recovery. The weekly timer is enabled. The following initialization step is for a fresh repository.

Initialize the repository explicitly after checking its endpoint and bucket. Install [backup.service](../../deploy/backup.service) and [backup.timer](../../deploy/backup.timer) as `nurevolution-backup.service` and `nurevolution-backup.timer`. Create `/var/cache/nurevolution-restic`, ensure the paths in the service are present, and test the service before enabling its timer. Install the [stable tooling bootstrap](tooling-bootstrap.md) before using the current backup unit. The bootstrap selects the current release tool and coordinates with promotion/rollback through a tooling lock. The backup command then takes site/edge locks, uses one read worker and bounded Go memory/CPU, backs up media/release state/profile/tooling and edge config/ACME data, checks repository structure, then applies retention. It records the snapshot ID only after success. The tooling tree includes retained release executables, original GitHub artifact ZIPs, provenance receipts, and the active selector; retain them together for verified offline rollback. Bootstrap/site/edge `deploy.lock` directories are excluded. The separately installed bootstrap and wrapper can be restored from the pinned reviewed repository revision. Restic encryption and content hashing apply to uploads; `check` alone is not a full-data restore test.

On a replacement host, also provision the [mise-managed Node runtime](node-runtime.md) and recreate the root-owned `node-runtime.json` before invoking the bootstrap's backup or rollback commands. These machine installation files and `/opt/nurevolution/mise` are outside the application backup. They are reproducible from the reviewed revision; retain the selected Node version with the host's recovery records. Restic itself can restore the application snapshot before Node is available.

All repository operations use two S3 connections, an 8 MiB target pack size, `GOMAXPROCS=1`, and `GOMEMLIMIT=96MiB`. Keep these bounds when running restore/check commands manually on this droplet. The full-archive rehearsal hit the service's 192 MiB cgroup limit with restic's default five connections and 16 MiB packs; the Go heap limit alone did not bound the complete process. [Restic's tuning guide](https://restic.readthedocs.io/en/stable/047_tuning_parameters.html) explains the additional buffer cost of connections and pack size. The revised limits completed the full archive rehearsal. Operation alongside the running application passed the live rehearsal. The success record uses the snapshot ID returned by this backup's JSON summary, so a separate rehearsal snapshot cannot be mistaken for the new application backup.
...
Trigger the same backup service after content changes and require its recorded successful snapshot before publication is considered complete. Weekly backups bound ordinary configuration/state loss to about seven days; newly published content has an explicit backup checkpoint. The owner accepted a four-hour host recovery target at [M6 closure](../milestones/evidence/M06-retirement.json) on 2026-09-18. Existing-host restores are verified; replacement-host provisioning and recovery duration remain unmeasured. Rehearsal against this objective is an ongoing operations follow-up, not an M6 blocker.
...
The owner confirmed the setup message, then approved one controlled failure test. [Recorded evidence](../milestones/evidence/M05-backup-alert-wiring.json) shows the installed backup unit triggering the installed notifier, Discord confirming the saved test message, and both normal service configurations being restored. The normal application/edge backup passed and the weekly timer is enabled. A source-workstation copy or same-disk staging directory does not establish independent restore readiness.
