...
The deployment renderer sets `apps.http.servers.https.protocols` to `["h1", "h2"]` and removes that server's `listen_protocols` overrides, which otherwise take precedence. This applies to existing configurations when a verified release is promoted, as well as new hosts using the initial example. It affects all routes on the shared `https` server; other servers, unrelated routes, TLS automation and listener addresses are preserved. A separate nonterminal `@id: nurevolution-transport` route immediately before the site scopes a deferred `Alt-Svc: clear` header to the site's hosts. Successful TCP responses clear previously advertised alternatives, including on redirects, direct media and proxied responses.
...
Explicit rollback to a pre-mitigation release uses that release's exact tooling. Those older tools replace only the `@id: nurevolution` site route, preserving both the listener protocol settings and the separate transport route. Cache clearing therefore remains active after rollback. Repeated promotions update the transport route without duplicating it.
...
The delivery gate checks the initial TLS listener's sockets and the generated cache-clearing header on media HEAD/range requests, redirects and proxied feed responses. It also reloads a predecessor-style site route without cache-clearing middleware, then verifies that the separate policy still sends `Alt-Svc: clear` and QUIC stays disabled. Deployment tests cover existing HTTP/3 and per-listener settings, preserve unrelated servers, and exercise the normal promotion path.
