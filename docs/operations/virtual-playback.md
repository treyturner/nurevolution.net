...
The capacity check builds the current runtime, warms every VBR header, runs 16 paced streams for 120 seconds in a read-only 256 MiB container, includes abandoned responses, and loads health, SSR and RSS concurrently. It records resident memory, completed checks and OOM state in `.local/playback/capacity.json`. Use the workspace's configured Docker socket and follow [Docker socket and test HTTP setup](../DEPLOYMENT.md#release-and-host-contracts) if published test ports require `NUREVOLUTION_DOCKER_ADDRESS`. This variable controls the HTTP bind/request address, not the Docker connection. Confirm the daemon can bind the specified original-audio directory.
