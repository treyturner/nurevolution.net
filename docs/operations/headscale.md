...
- The owner prefers LAN access to containers and has not requested additional Unraid firewall rules. The dedicated subnet does not promise isolation from other routed networks. A disposable test showed the Headscale-side container could reach a separate `nat-unprotected` peer; no additional Unraid firewall rules are installed. Headscale enrollment and policy do not advertise the Docker/LAN networks.
...
2. The owner adds the prepared service to the existing Unraid Compose project using the prepared fixed address `172.26.0.2`. Inspect existing services and names before creating anything.
