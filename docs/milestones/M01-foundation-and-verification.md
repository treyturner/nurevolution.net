...
Setup, commands, report locations, dependency rationale, and audio lifecycle behavior were originally documented in the root README and are now maintained in the [development guide](../DEVELOPMENT.md). `.nuxtrc` records the already configured test-utils version to avoid creating a setup marker during verification. Generated reports and local working evidence are ignored by Git.
