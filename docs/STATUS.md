...
1. **M8: listening and feed enhancements.** All five areas and the subsequent UI refinements are consolidated into [PR #56](https://github.com/treyturner/nurevolution.net/pull/56) at the owner's request. The earlier Safari-only head passed CI and automatic review; the expanded PR passed its full local gate (560 unit/integration tests, 356 browser tests, four expected skips, and delivery checks) and awaits fresh remote CI/review. The owner confirmed iPad playback, seeking, both reported visual fixes, downloading, and Download MP3 -> View. Dialog/restoration/guest-episode checks, physical device controls, and podcast-client artwork/chapter acceptance remain pending. The workspace preview includes current-origin timestamp links, the playhead context menu, inline track-copy controls, and responsive player refinements. **RSS chapters are not yet deployed.** The resource-support fallback must still be published, deployed, and verified before the RSS-advertisement release; consolidating review does not remove that rollout requirement. No M8 deployment is authorized. [Implementation plan and acceptance](milestones/M08-listening-and-feed-enhancements.md).
