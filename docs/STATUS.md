...
1. **M8: listening and feed enhancements.** Implementation is underway, beginning with Safari 14.3-compatible startup, dialogs, layout, and initialization feedback. Actual iPad acceptance remains pending. Timestamp sharing, supported lock-screen/headset controls, episode RSS artwork, and feed chapters follow in separate review slices; all five areas are required. **RSS chapters are not emitted today.** No M8 deployment is authorized. [Implementation plan and acceptance](milestones/M08-listening-and-feed-enhancements.md).
