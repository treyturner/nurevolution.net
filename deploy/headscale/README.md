...
Keep credential values out of chat, shell arguments, logs, commits, and artifacts. The droplet consumes its key with `--auth-key=file:/run/nurevolution-enrollment/droplet.key`; the runner key is used for the disposable client rehearsal and then stored as the deployment environment's `HEADSCALE_AUTH_KEY` secret. The owner confirmed the current runner key as ID 2, expiring on **2026-12-10 at 17:27:47 UTC**. Production now uses this key; preview is disabled. The owner prefers avoiding routine renewal and accepts a long-lived Actions enrollment secret. A replacement using the verified `876000h` duration is recommended but not yet installed; the original expiry still applies. Headscale 0.29.3 does not provide a supported never-expiring creation option. Follow [environment configuration and replacement](../../docs/operations/headscale.md#github-environment-and-rotation) for every enabled consumer. Neither key grants Headscale administrator access.
