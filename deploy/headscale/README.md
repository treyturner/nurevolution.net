...
Keep credential values out of chat, shell arguments, logs, commits, and artifacts. The droplet consumes its key with `--auth-key=file:/run/nurevolution-enrollment/droplet.key`; the runner key is used for the disposable client rehearsal and then stored as the deployment environment's `HEADSCALE_AUTH_KEY` secret. The current runner key is **ID 3**, with owner-confirmed expiry **2126-08-19 at 23:29:04 UTC**. The production GitHub environment secret was updated and production verification passed. The owner prefers avoiding routine renewal and accepts a long-lived Actions enrollment secret. This uses the verified `876000h` duration; Headscale 0.29.3 does not provide a supported never-expiring creation option. The [replacement record](../../docs/milestones/evidence/M06-headscale-key-replacement.json) tracks key 2 revocation and the subsequent backup. Follow [environment configuration and replacement](../../docs/operations/headscale.md#github-environment-and-rotation) for future replacements. Neither key grants Headscale administrator access.
